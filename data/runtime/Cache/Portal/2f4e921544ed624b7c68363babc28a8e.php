...
<body>
			...
			<div class="row-fluid">
				<div class="span9">
					<table class="table table-bordered">
						...
						<?php if($showaddress == 1 ): $getaddress=sp_getaddress(); ?>
					     <tr>
							<th width="80">类别</th>
							<td>
							 <select style="max-height: 100px;"  name="postpage[type]" id="changeType" >
							 	<?php if('商务' == $_SESSION['pagetype']): ?><option value="婚礼" >婚礼</option>
								 <option value="商务" selected="">商务</option>					 
									<?php else: ?>
									<option value="婚礼" selected="">婚礼</option>
								 <option value="商务">商务</option><?php endif; ?>
					    ...
					    <tr>
							<th width="80">地址</th>
							<td>
							<select style="max-height: 100px;"  name="post[postadderss]" id="changeAddress">
							<?php if(is_array($getaddress)): foreach($getaddress as $key=>$vo): ?><option value="<?php echo ($vo["address"]); ?>"  <?php if($vo["address"] == $_SESSION['pageaddress']): ?>selected=""<?php endif; ?>><?php echo ($vo["address"]); ?></option><?php endforeach; endif; ?>
							 </select> 					 
							</td>							 
						</tr>  
						<tr>
							<th>内容</th>
							<td id="changeCon">
								<script type="text/plain" id="content" name="postpage[post_content]"><?php echo ($postpage["post_content"]); ?></script>
							</td>
						</tr> 
						 <?php else: ?>
						<tr>	
	...
	<script type="text/javascript">
		$(function() {
			$("#changeAddress,#changeType").change(function(){
				$.post("<?php echo U('Portal/AdminPage/changeAds');?>",{postadderss:$("#changeAddress").val(),pid:'<?php echo ($post["id"]); ?>',type:$("#changeType").val()},function(data){
					if(data==1){	
