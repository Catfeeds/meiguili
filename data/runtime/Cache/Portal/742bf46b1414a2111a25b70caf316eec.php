...
<div class="w1200 mtb50 clear">
    ...
    <div class="list-activity clear">
        <?php if($_SESSION["mainnav"]==2){ $lists = sp_sql_posts_paged("cid:$cat_id;order:listorder DESC;where:type='商务' and postadderss='$address'",5); }else{ $lists = sp_sql_posts_paged("cid:$cat_id;order:listorder DESC;where:type='婚礼' and postadderss='$address'",5); } ?>
        <?php if(is_array($lists['posts'])): $i = 0; $__LIST__ = $lists['posts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $smeta=json_decode($vo['smeta'], true); ?>
            ...
            <div class="list-boxes clear">
                ...
                <div class="list-act-r">
                    <h2><?php echo ($vo["post_title"]); ?><span class="meta"><?php echo date('Y-m-d',strtotime($vo['post_date']));?></span></h2>
                    <p><img src="/images/act-icons_01.png">活动时间:<?php echo date('Y/m/d',strtotime($vo['actstarttime']));?>-<?php echo date('Y/m/d',strtotime($vo['actendtime']));?>
                    </p>
                    <p><img src="/images/act-icons_03.png">报名人数:<span class="red"><?php echo ($vo["newarc"]); ?></span>人</p>
                    <p><img src="/images/act-icons_05.png">人气指数:<span class="red"><?php echo ($vo["post_hits"]); ?></span>次</p>
                    <div class="act-icons-time">

                        <?php $datatime=time(); $starttime=strtotime ($vo['actstarttime']); $endtime=strtotime ($vo['actendtime']); ?>
                        <?php if($datatime < $starttime): ?><span class="color2">活动未开始</span>
                            <?php elseif($starttime < $datatime &&$datatime < $endtime): ?>
                            <a href="<?php echo leuu('article/index',array('id'=>$vo['tid'],'cid'=>$vo['term_id']));?>"><span
                                    class="color1">活动报名中</span>
                            </a>
                            <?php else: ?>
                            <span class="color2">活动已结束</span><?php endif; ?>
