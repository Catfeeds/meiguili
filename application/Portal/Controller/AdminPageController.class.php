...
class AdminPageController extends AdminbaseController {
    ...
    public function edit(){
        ...
        if($id==7||$id==8||$id==9){
            $showaddress=1;           
            if($_SESSION['pageaddress']){
				$data['postadderss']=$_SESSION['pageaddress']; 
            }else{
            	$data['postadderss']='北京·太阳公元'; 	
            }  
            
            if($_SESSION['pagetype']){
				$data['type']=$_SESSION['pagetype']; 
            }else{
            	$data['type']='婚礼';
            }  
            $data['pid']=$id;
            $page_obj = M("Page"); //var_dump($data)  ;exit();
            $postpage=$page_obj->where($data)->find(); // var_dump($postpage)  ;exit(); 
        ...
        $this->assign("showaddress",$showaddress);
        $this->display();
    }
    ...
    public function edit_post(){
        ...
        if (IS_POST) {
            ...
            if($page['id']==7||$page['id']==8||$page['id']==9){
                $page_obj = M("Page");
                $datapage['pid']=$page['id'];
                $datapage['postadderss']=$page['postadderss'];
                $datapage['type']=$_SESSION['pagetype'];              
                $findpage=$page_obj->where($datapage)->find();
                $postpage=I("post.postpage");
                if($findpage){
                    $enddata['post_content']=htmlspecialchars_decode($postpage['post_content']);                   
                    $pageresult=$page_obj->where($datapage)->save($enddata);
                }else{
	                $datapage['post_content']=htmlspecialchars_decode($postpage['post_content']);
	                $pageresult=$page_obj->add($datapage);
                }
    ...
    public function changeAds(){
    	if (IS_POST) {
    		$_SESSION['pageaddress']=$_POST['postadderss'];
    		$_SESSION['pagetype']=$_POST['type'];
