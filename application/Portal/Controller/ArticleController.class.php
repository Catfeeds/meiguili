...
class ArticleController extends HomebaseController {
    ...
    public function index() {
    	...
    	$newarc_model=M("Newarc");
        $countid['arcid']=$article_id;
    	$countno=$newarc_model->where($countid)->count();  
     ...
     public function yyadd(){
        $this->check_login();
         if (IS_POST) {
                ...
                $newarc_model=M("Newarc");
                $where['uid']= $posttime['uid'];
                $where['arcid']=$_POST['arcid'];
                $findrst=$newarc_model->where($where)->find(); 
                if($findrst){
                    $this->error("该活动您已经报过名！");
                }else{
                   $rst = $newarc_model->add($data);
                    if($rst){ 
                    $posts_model=M("Posts");
                    $posts_model->save(array("id"=>$_POST['arcid'],"newarc"=>array("exp","newarc+1")));                                               
                    $this->success("报名成功！");            
                    }else{
                    $this->error("报名失败！");
                    }   
